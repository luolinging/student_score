...
//学生选课
public class ScDAO {
	public Vector ScSelect(String cno, String cname, double credit,
			int cteachno, String clocation, String ctime, String tname)
			throws Exception {
	...
	public Vector SelectSelf(String sid) throws Exception {
		Vector all = new Vector();
		String sql = "SELECT sc.cno, course.cname, sc.credit, tc.cteachno, tc.clocation, tc.ctime, teacher.tname FROM sc,tc,course,teacher where sc.sid='"
				+ sid
				+ "' and sc.cno=course.cno and sc.cno=tc.cno  and sc.tid=tc.tid and tc.tid=teacher.tid;";
		Statement stmt = null;
		DataBaseConnection dbc = null;
		dbc = new DataBaseConnection();
		try {
	...
	public Vector SelectStudent(String cno, String tid) throws Exception {
		Vector all = new Vector();
		String sql = "SELECT sc.sid, student.sname, sc.credit, sc.pscore, sc.lscore,sc.escore FROM sc,student where sc.cno='"
				+ cno
				+ "' and sc.tid='"
				+ tid
				+ "' and sc.sid=student.sid ORDER BY sc.escore DESC;";
		Statement stmt = null;
		DataBaseConnection dbc = null;
		dbc = new DataBaseConnection();
		try {
	...
	public Vector SelectCourse(String sid) throws Exception {
		Vector all = new Vector();
		String sql = "SELECT sc.cno,course.cname, sc.tid, sc.credit, sc.pscore, sc.lscore,sc.escore FROM sc,course where sc.sid='"
				+ sid + "' and sc.cno=course.cno;";
		Statement stmt = null;
		DataBaseConnection dbc = null;
		dbc = new DataBaseConnection();
		try {
	...
	public void Insert(String sid, String cno, String tid, double credit)
			throws Exception {
		String sql = "INSERT INTO sc(sid,cno,tid,credit) VALUES('" + sid
				+ "','" + cno + "','" + tid + "','" + credit + "');";
		Statement stmt = null;
		DataBaseConnection dbc = null;
		dbc = new DataBaseConnection();
	...
	public Score GetMaxMinAvg(String cno, String tid) throws Exception {
		Score sco = new Score();
		String sql = "SELECT AVG(escore),MAX(escore),MIN(escore) FROM sc WHERE cno='"
				+ cno + "' and tid='" + tid + "';";
		Statement stmt = null;
		DataBaseConnection dbc = null;
		dbc = new DataBaseConnection();
	...
	public void UpdateScore(String sid, String cno, String tid, double pscore,
			double lscore) throws Exception {
		double escore = pscore * 0.3 + lscore * 0.7;
		double scorejd = calxfjd(escore);
		String sql = "UPDATE sc set pscore=" + pscore + ",lscore=" + lscore
				+ ",escore=" + escore + ",scorejd=" + scorejd + " WHERE sid='"
				+ sid + "' and cno='" + cno + "' and tid='" + tid + "';";
		Statement stmt = null;
	...
	public boolean Check(String sid, String ctime) throws Exception {
		boolean b = false;
		String sql = "SELECT tc.ctime FROM tc,sc WHERE  sc.cno=tc.cno and sc.sid='"
				+ sid + "' and tc.ctime='" + ctime + "';";
		Statement stmt = null;
		DataBaseConnection dbc = null;
		dbc = new DataBaseConnection();
