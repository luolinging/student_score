...
//教师开设的课程
public class TcDAO {
	public void InsertCourse(String tid, String cno, int cteachno,
			String clocation, String ctime, int cmax) throws Exception {
		String sql = "INSERT INTO tc(tid,cno,cteachno,clocation,ctime,cmax,csel,sub) VALUES('"
				+ tid
				+ "','"
				+ cno
				+ "',"
	...
	public Vector SelectSelf(String tid, String con) throws Exception {
		...
		String sql = "SELECT tc.cno,course.cname,course.credit,tc.cteachno,tc.clocation,tc.ctime,tc.cmax,tc.csel from tc,course where course.cno=tc.cno and tc.tid='"
				+ tid + "' ";
		if (con.equals("1")) {
	...
	public Vector SelectScore(String tid) throws Exception {
		Vector all = new Vector();
		String sql = "SELECT tc.cno,course.cname,course.credit,tc.csel from tc,course where tc.tid='"
				+ tid + "' and course.cno=tc.cno;";
		Statement stmt = null;
		DataBaseConnection dbc = null;
		dbc = new DataBaseConnection();
		try {
	...
	public String SelectCtime(String cno, int cteachno) throws Exception {
		String sql = "SELECT ctime from tc where cno='" + cno
				+ "' and cteachno=" + cteachno + ";";
		Statement stmt = null;
		DataBaseConnection dbc = null;
		dbc = new DataBaseConnection();
		String tid = null;
	...
	public void AddCsel(String tid, String cno) throws Exception {
		String sql = "UPDATE tc set csel=csel+1 WHERE tid='" + tid
				+ "' and cno='" + cno + "';";
		Statement stmt = null;
	...
	public String SelectSub(String tid, String cno) throws Exception {
		String sub = null;
		String sql = "SELECT sub FROM tc WHERE tid='" + tid + "' and cno='"
				+ cno + "'";
		Statement stmt = null;
		DataBaseConnection dbc = new DataBaseConnection();
		try {
	...
	public void UpdateSub(String tid, String cno, String con) throws Exception {
		String sql = "UPDATE tc set sub='" + con + "' WHERE tid='" + tid
				+ "' and cno='" + cno + "';";
		Statement stmt = null;
		DataBaseConnection dbc = new DataBaseConnection();
		try {
